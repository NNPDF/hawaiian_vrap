...
def run_comparison(dataset_vp, dataset_pin, pdf_name = "NNPDF40_nnlo_as_01180", ratio = 1.0, verbose=True, mass_divide=False, remove_points=False):
    ...
    if isinstance(dataset_pin, str):
        if "*" in dataset_pin:
            dataset_pin = [i.replace(".pineappl.lz4","") for i in sorted(glob(dataset_pin + ".pineappl.lz4"))]
        else:
    ...
    for d in dataset_pin:
        ...
        if remove_points:
            ratio_mass *= (np.less(grid.raw.bin_left(0), 7.52) | np.greater(grid.raw.bin_left(0), 7.54))
        res_pine_partial.append(grid.convolute_with_one(2212, lpdf.xfxQ2, lpdf.alphasQ2)*ratio_mass)
...
# And now put them both side by side
    ...
    if ratio != 1.0:
        ...
        all_res_corrected.to_csv(f"out_csv/{dataset_vp}_corr.csv", sep="\t", header=["vp", f"pine*{ratio}", "ratio"])
